...
To control for the baseline effects of social factors like race and socioeconomic status and other health metrics like triglyceride and cholesterol levels, we regress "preterm" against all other variables. We find that dde, totalpcb, triglycerides, race, and center 37 are statistically significant. We do the same with "earlypreterm" and find only totalpbc and race to be significant. The tables below provide effect size and pvalues for DDE and PCB, the variables of interest.
