...
abstract: "We assess the effect of dichlorodiphenyldichloroethene (DDE) and polychlorinated biphenyls (PCBs) on the risk of preterm birth for pregnant women while controlling for socio-economic and health related variables. Since we treat preterm birth as a binary response (preterm/non-preterm birth), we use a logistic regression model, a common statistical model for modeling binary response data under a set of parametric assumptions. In addition, we attempt to validate results from the logistic regression model using Bayesian model averaging.  Our analysis indicates a potential association between DDE, PCB, and risk of preterm birth, though any causal conclusions are limited. To explore non-linear dependencies, a comparison between the logistic regression and a random forest model is given in the appendix."
output: 
  pdf_document:
    keep_tex: true
    fig_caption: true
...
```{r, echo=FALSE, fig.width=4, fig.height=1.8, fig.align="center", fig.cap="Multiplicative effect of dde and totalpcb on the odds ratio of preterm to non-preterm birth, as a function of the empirical dde or pcb concentration quantile. Confidence intervals are represented by shaded regions, and the plot is truncated on the right at the 0.99 quantile."}
