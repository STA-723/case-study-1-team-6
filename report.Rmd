...
We used a logistic regression to estimate the effect of *dde* and *totalpcb* on preterm risk, controlling for socio-economic and health-related variables. This allowed us to display a significant association between presence of these chemicals and preterm risk: higher concentrations increased the estimated risk of preterm birth. 
...
Finally, we reproduce Figure 1, which shows preterm risk as a function of range of chemical concentrations, adding to it the empirical very preterm risks. There is no clear marginal relationship between chemical concentrations and very preterm risks.
...
```{r,echo=FALSE, warning=FALSE, fig.height=5, cache= TRUE}
data_i = data_i %>% mutate(earlypreterm = preterm < 34)
#Models for early preterm
model.base.early <- glm(earlypreterm ~ dde + totalpcb + triglycerides + race + score_education + score_income_i + score_occupation_i + maternal_age + cholesterol + center, data = data_i, family = "binomial")

bas.model.base.early <- bas.glm(earlypreterm ~ dde + totalpcb + triglycerides + race + score_education_i + score_income_i + score_occupation_i + maternal_age + cholesterol + center, data = data_i, family = binomial(link = "logit"), modelprior = uniform(), MCMC.iterations = 10000)
